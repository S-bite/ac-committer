...
void State::update()
{
    if (curNode == 0 && curDist == 0)
    {
        ...
        if (stackItem.size() >= deliverNum[deliverNumIndex] || (!stackItem.empty() && curTime > 95 * maxTime / 100))
        {
...
int simulate(State &s, vector<Item> &orders)
{
    assert(s.curTime == 0);
...
State genRandomState(int orderNum)
{
...
int main()
{
    ...
    while (double(clock() - starttime) / CLOCKS_PER_SEC < 29.0)
    {
        ...
        int newScore = simulate(newState, orders);
