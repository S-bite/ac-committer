...
//vで指定された{注文時間,頂点（同じものが複数ある場合もある）}のペアから、貪欲法で短いパスを求め、その順番（重複はない）を返す
vector<int> genGreedyPath(vector<pair<int, int>> &v)
{
    //vから同じ頂点を省いたもの
...
void genDeliverPathFromStackItem(vector<int> &deliverPath, vector<pair<int, int>> &stackItem, int maxTime, int curTime)
{
    ...
    vector<int> tmp;
    vector<int> greedyPath = genGreedyPath(stackItem);
