...
int predict1(vector<int> &s)
{
    if (s.size() < 100)
    {
        return mt() % lastTime + 1;
    }
    int splitTime = s.size() / 2;
    lld slope = (lld)((s.back() - s[slope]) - (s[slope] - s[0])) / ((lld)splitTime * splitTime * splitTime);
    int pPeakTime1 = mt() % lastTime + 1;
    if (slope != 0.0)
    {
        pPeakTime1 = (int)(1.0 / slope);
        if (pPeakTime1 < 0)
            pPeakTime1 = 0;
...
int main()
{
    ...
    while (state.curTime < maxTime)
    {
        ...
        if (state.curTime > 1000 && abs(p1 - p2) > abs(pp1 - p2))
        {
            p1 = pp1;
            p2 = pp2;
        }
