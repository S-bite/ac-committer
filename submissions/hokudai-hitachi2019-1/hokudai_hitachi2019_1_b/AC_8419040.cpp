...
void genDeliverPathFromStackItem(vector<int> &deliverPath, vector<pair<int, int>> &stackItem, int maxTime, int curTime)
{
    ...
    vector<int> tmp;
    map<int, int> weight; //目的地に対する訪れるべき度
    for (auto t : stackItem)
    {
        ...
        //最速で届けた場合に得られる点数
        weight[t.second] += maxTime * maxTime - (curTime + cost[0][t.second] - t.first) * (curTime + cost[0][t.second] - t.first);
        assert(weight[t.second] > 0);
    }
    vector<int> deliverItem;
    int deliverLimit = 130;
    //dumpVector(tmp);
...
public:
    ...
    void update()
    {
        ...
        {
            if (!stackItem.empty()) // if (stackItem.size() >= 25 || (stackItem.size() > 0 && curTime >= (maxTime * 8) / 10))
            {
