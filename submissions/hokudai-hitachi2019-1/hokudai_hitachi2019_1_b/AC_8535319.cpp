...
int main()
{
    ...
    int p1 = predict1(actualOrderSum);
    int p2 = predict2(actualOrderSum);
    int pp1 = p1, pp2 = p2;

    cerr << actualOrderSum.size() << endl;
    genOrderNumAndSumFromPeakTime(7760, pseudoOrderNum, pseudoOrderSum);
    ...
    while (state.curTime < maxTime)
    {
        ...
        p1 = predict1(actualOrderSum);
        p2 = predict2(actualOrderSum);
        if (abs(p1 - pp1) > 1000 && pp1 != -1)
            p1 = pp1;
        if (abs(p2 - pp2) > 1000 && pp2 != -1)
            p2 = pp2;

        if (p1 != -1 && p2 != -1)
        {
            if (state.curTime > p1 && abs(p1 - p2) > abs(pp1 - pp2))
            {
                f = true;
            }
            //cerr << state.curTime << " ";
            if (f)
            {
                genOrderNumAndSumFromPeakTime(p2, pseudoOrderNum, pseudoOrderSum);
                //cerr << "p2 " << p2 << "p1 " << p1 << endl;
            }
            else
            {
                genOrderNumAndSumFromPeakTime(p1, pseudoOrderNum, pseudoOrderSum);
