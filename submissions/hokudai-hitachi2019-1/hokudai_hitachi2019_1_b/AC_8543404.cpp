...
void State::update()
{
    deliverNumIndex = actualOrderSum.size() / 100;
    if (curNode == 0 && curDist == 0)
    {
        ...
        if (!stackItem.empty())
        {
            ...
            genDeliverPathFromStackItem(deliverPath, deliverItem, stackItem, maxTime, curTime, deliverNum[deliverNumIndex]); //+ max(pseudoOrderSum[max(curTime - 10, 0)] - deliveredItemNum, 0));
...
int main()
{
    ...
    cin >> maxTime;
    pseudoOrderSum = vector<int>(maxTime, 0);
    ...
    while (state.curTime < maxTime)
    {
        state.sync();
        //cerr << "SYNC" << endl;
        if (false && state.curTime % 50000 == 0 && double(clock() - starttime) / CLOCKS_PER_SEC < 27)
        {
