...
struct State
{
    ...
    double weighted_profit(int y, int x)
    {
        ...
        rep(i, SIZE) rep(j, SIZE)
        {
            ...
            ret = max(ret, (double)grid[i][j] - (double)C * 0.5 * (fabs(i - y) + fabs(j - x)));
        }
        // cerr << "ret " << ret << endl;
        return ret;
    }
...
int main()
{
    ...
    rep(i, SIZE)
    {
        pque.push({sim.weighted_profit(0, i) - sim.mine_cost(0, i), 0, i});
        pque.push({sim.weighted_profit(SIZE - 1, i) - sim.mine_cost(SIZE - 1, i), SIZE - 1, i});
        pque.push({sim.weighted_profit(i, 0) - sim.mine_cost(i, 0), i, 0});
        pque.push({sim.weighted_profit(i, SIZE - 1) - sim.mine_cost(i, SIZE - 1), i, SIZE - 1});
    }
    VI score;
    ...
    while (!pque.empty())
    {
        ...
        rep(i, 4)
        {
            ...
            pque.push({sim.grid[ny][nx] + sim.weighted_profit(ny, nx) - sim.mine_cost(ny, nx), ny, nx});
        }
    }
    int best_score = 0;
    int best_idx = -1;
