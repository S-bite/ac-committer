...
int main()
{
    ...
    vector<PII> candid;
    rep(i, 40, 60 + 1) rep(j, 40, 60 + 1)
    {
        if (board[i][j - 1] == board[i][j + 1] || board[i - 1][j] == board[i + 1][j] || board[i - 1][j] == board[i][j] || board[i][j - 1] == board[i][j])
        {
            candid.push_back({i, j});
        }
    }
    shuffle(ALL(candid), mt);
    cerr << candid.size() << endl;
    int sz = 12;
    /*rep(dx, sz) rep(dy, sz)
    {
        int i = n / 2 - sz / 2 + dy;
    ...
    rep(t, min((int)candid.size(), 140))
    {
        int i = candid[t].first;
        int j = candid[t].second;
        int cur = test(board, i, j);
        if (cur > score)
        {
            score = cur;
