...
void move(int target)
{
    ...
    while (pos[target].first > state.y) {
        ...
        if (state.y + 1 < N && state.grid[state.y][state.x] != -1 && state.grid[state.y + 1][state.x] == -1 && state.y < N / 2) {
            I();
            pickOtherCard = true;
        }
        cout << "D";
    ...
    while (pos[target].first < state.y) {
        bool pickOtherCard = false;
        if (state.y - 1 >= 0 && state.grid[state.y][state.x] != -1 && state.grid[state.y - 1][state.x] == -1 && state.y > N / 2) {
            I();
            pickOtherCard = true;
        }
        cout << "U";
    ...
    while (pos[target].second > state.x) {
        bool pickOtherCard = false;
        if (state.x + 1 < N && state.grid[state.y][state.x] != -1 && state.grid[state.y][state.x + 1] == -1 && state.x < N / 2) {
            I();
            pickOtherCard = true;
        }
    ...
    while (pos[target].second < state.x) {
        bool pickOtherCard = false;
        if (state.x - 1 >= 0 && state.grid[state.y][state.x] != -1 && state.grid[state.y][state.x - 1] == -1 && state.x > N / 2) {
            I();
            pickOtherCard = true;
        }
