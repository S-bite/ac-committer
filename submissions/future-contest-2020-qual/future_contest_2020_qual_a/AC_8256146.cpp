...
int main(int argc, char const *argv[])
{
    ...
    uniform_real_distribution<> unirnd(0.0, 1.0);
    int gy, gx;
    cin >> n >> m >> b;
    cin >> gy >> gx;
    ...
    int cnt = 0;
    //double C = 3.0;
    //double endtime = ((double)starttime / CLOCKS_PER_SEC) + 3.0;
    while ((double(clock() - starttime) / CLOCKS_PER_SEC < 2.9))
    {
        ...
        //double force = (endtime - ((double)clock() / CLOCKS_PER_SEC)) / C;
        //cerr << (force > unirnd(mt)) << endl;
        if (ans.score < newans.score) // || (force > unirnd(mt) && newans.score != -1000))
        {
