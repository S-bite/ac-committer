...
void fillsign1(vector<vector<char>> &signs, vector<vector<bool>> &blocks, int y, int x, char c)
{
    signs[y][x] = c;
    if (isvalid(y + 1, x) && blocks[y + 1][x] == false && signs[y + 1][x] == ' ')
    {
        fillsign1(signs, blocks, y + 1, x, 'U');
    }

    if (isvalid(y - 1, x) && blocks[y - 1][x] == false && signs[y - 1][x] == ' ')
    {
        fillsign1(signs, blocks, y - 1, x, 'D');
    }
    if (isvalid(y, x + 1) && blocks[y][x + 1] == false && signs[y][x + 1] == ' ')
    {
        fillsign1(signs, blocks, y, x + 1, 'L');
    }
    if (isvalid(y, x - 1) && blocks[y][x - 1] == false && signs[y][x - 1] == ' ')
    {
        fillsign1(signs, blocks, y, x - 1, 'R');
    }
}
void fillsign2(vector<vector<char>> &signs, vector<vector<bool>> &blocks, int y, int x, char c)
{
...
int main(int argc, char const *argv[])
{
    ...
    Answer ans;
    init(ans);
    fillsign2(ans.signs, blocks, gy, gx, '@');
    ans.signs[gy][gx] = ' ';
    rep(i, n) rep(j, n)
    {
        if (ans.signs[i][j] != ' ')
    ...
    while ((double(clock() - starttime) / CLOCKS_PER_SEC < 2.9))
    {
        ...
        double delta = ans.score - newans.score;
    ...
    printans(ans);
    cerr << "score:" << ans.score << " cnt:" << cnt << endl;
    return 0;
}
