...
bool operator<(const Answer &ans1, const Answer &ans2)
{
    return ans1.score < ans2.score;
};
...
void fillsign(vector<vector<char>> &signs, vector<vector<bool>> &blocks, int y, int x, char c, int p)
{
    ...
    while (!que.empty())
    {
        ...
        rep(i, 4)
        {
            ...
            if ((p + i) % 4 == 0 && blocks[(y + 1 + n) % n][x] == false && signs[(y + 1 + n) % n][x] == ' ')
            {
                que.push_back({(y + 1 + n) % n, x, 'U'});
            }
            if ((p + i) % 4 == 3 && blocks[y][(x - 1 + n) % n] == false && signs[y][(x - 1 + n) % n] == ' ')
            {
                que.push_back({y, (x - 1 + n) % n, 'R'});
            }

            if ((p + i) % 4 == 1 && blocks[(y - 1 + n) % n][x] == false && signs[(y - 1 + n) % n][x] == ' ')
            {
                que.push_back({(y - 1 + n) % n, x, 'D'});
            }
            if ((p + i) % 4 == 2 && blocks[y][(x + 1 + n) % n] == false && signs[y][(x + 1 + n) % n] == ' ')
            {
...
int main(int argc, char const *argv[])
{
    ...
    priority_queue<Answer, vector<Answer>> pq;
    ...
    rep(i, 4)
    {
        ...
        Answer ans;
        init(ans);
        fillsign(ans.signs, blocks, gy, gx, '@', i);
        ans.signs[gy][gx] = ' ';
        rep(i, n) rep(j, n)
        {
            if (ans.signs[i][j] != ' ')
    ...
    printans(ans);
    cerr << ans.score << " (" << ans.movedScore << ") " << cnt << endl;
    return 0;
}
