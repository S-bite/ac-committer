...
int main(){
  ...
  pair<int, pair <int,int > > a[D];
  for(int i=0;i<D;i++){
    a[i].first=(((i+1)*100*p[i]+c[i])/p[i]);
    a[i].second.first=(i+1)*100*p[i]+c[i];
    a[i].second.second=i;
  }
  bool used[D]={false};
  sort(a,a+D,greater<pair<int, pair <int,int> > >());
  int ans=0;
  for(int i=0;i<D;i++){
  ...
  for(int i =0;i<D;i++){
    ...
    else if(G>(i+1)*100*(p[i]-1) and G<=(i+1)*100*p[i]+c[i]) _min=min(_min,p[i]);
  }
  cout<<_min+ans<<endl;
}
