...
public:
    ...
    void sync();
    void update();
    void canselOrder(int);
};
void State::canselOrder(int id)
{
    int dbg = 0;
    ...
    rep(i, stackItem.size())
    {
        if (stackItem[i].id == id)
        {
            stackItem.erase(stackItem.begin() + i);
            dbg++;
            break;
        }
    }
...
void initGraph()
{
    ...
    exinfo = vector<vector<int>>(v, vector<int>(v, INF));
...
int main()
{
    ...
    vector<vector<int>> delay(4, vector<int>(4));
    rep(i, 4) rep(j, 4) cin >> delay[i][j]; //iが方向
