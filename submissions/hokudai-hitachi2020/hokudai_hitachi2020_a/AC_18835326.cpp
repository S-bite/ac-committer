...
struct EV_is_battery : strategy<P>
{
    ...
    void command(const grid_info &g_i, const EV_info &ev_i, const order_info &)
    {
        for (size_t n = 0; n < ev_i.N_EV; ++n)
        {
            ...
            if (g_i.y[nanogrid_idx] < (double)S::grid.C_grid_max * 0.9)
            // if (g_i.pw_actual[nanogrid_idx] < 0)
            {
                size_t charge_V = min({S::EV.V_EV_max, ev_i.c[n].charge, S::grid.C_grid_max - g_i.y[nanogrid_idx]});
                if (charge_V > 0)
                {
            ...
            else
            {
                ...
                size_t charge_V = min({S::grid.V_grid_max, S::EV.V_EV_max, g_i.y[nanogrid_idx], S::EV.C_EV_max - ev_i.c[n].charge});
                if (charge_V > 0)
                {
                    cerr << "charge from grid" << endl;

                    DUMP(charge_V);
                    DUMP(S::EV.C_EV_max - ev_i.c[n].charge);
                    DUMP(S::EV.C_EV_max);
                    DUMP(ev_i.c[n].charge);

                    DUMP(g_i.y[nanogrid_idx]);
