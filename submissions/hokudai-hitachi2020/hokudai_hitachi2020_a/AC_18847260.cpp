...
struct graph_summary
{
    ...
    graph_summary(const graph_data &graph, const grid_data &grid) : len(graph.V, std::vector<size_t>(graph.V, 1e9)),
...
auto mincost_matching(const vector<size_t> &EV, const vector<size_t> &nanogrid, const graph_summary &gs, const vector<int> &loss)
{
    atcoder::mcf_graph<int, long long> g(EV.size() + nanogrid.size() + 2);
    int s = EV.size() + nanogrid.size();
    int t = EV.size() + nanogrid.size() + 1;
    //int place_num = min(EV.size(), nanogrid.size());
    int place_num = EV.size();
    int offset = EV.size();
    const int delta_move = 50;
    for (int i = 0; i < EV.size(); i++)
    {
        for (int j = 0; j < nanogrid.size(); j++)
        {
            assert(((int)gs.len[EV[i]][nanogrid[j]] * delta_move - loss[j]) / 100 + 1e6 > 0);
            g.add_edge(i, offset + j, 1, ((int)gs.len[EV[i]][nanogrid[j]] * delta_move - loss[j]) / 100 + 1e6);
            // g.add_edge(i, offset + j, 1, 1000000ll + gs.len[EV[i]][nanogrid[j]]);
        }
    }
    for (int i = 0; i < EV.size(); i++)
    {
    ...
    for (int j = 0; j < nanogrid.size(); j++)
    {
        g.add_edge(offset + j, t, 1, 0);
        g.add_edge(offset + j, t, 1, 1e9);
    }
    // for (int j = 0; j < (int)nanogrid.size(); j++)
    // {
    //     g.add_edge(offset + j, t, 1, 0);
    // }

    auto [flow, cost] = g.flow(s, t, place_num);
    assert(flow == place_num);
    vector<pair<int, int>> matching;
    for (auto edge : g.edges())
    {
        if (edge.flow != 1 || edge.from == s || edge.to == t)
        {
            continue;
        }
        matching.push_back({edge.from, edge.to - offset});
    }
    return matching;
}
...
vector<int> calc_loss(const grid_data &gd)
{
    vector<int> loss(gd.N_grid, 0);
    vector<int> cur_C(gd.N_grid, 0);
    int parretn_duration = 1000 / gd.N_div;
    for (int i = 0; i < gd.N_div; i++)
    {
        for (int j = 0; j < gd.N_grid; j++)
        {
            cur_C[j] += parretn_duration * gd.pw_predict[gd.pattern[j]][i];

            if (cur_C[j] > (int)gd.C_grid_max)
            {
                loss[j] += abs(cur_C[j] - (int)gd.C_grid_max);

                cur_C[j] = gd.C_grid_max;
            }

            if (cur_C[j] < 0)
            {
                loss[j] += 2 * abs(cur_C[j]);
                cur_C[j] = 0;
            }
        }
    }
...
template <class P>
struct EV_is_battery : strategy<P>
{
    using S = strategy<P>;
    map<size_t, size_t> pos_to_nanogrid;
    std::mt19937_64 engine;
    int total_loss;
    int total_buy;
    EV_is_battery(const P &p_, const graph_summary &gs) : strategy<P>(p_, gs)
    {
        total_loss = 0;
        total_buy = 0;
    }
    ...
    {
        cerr << "init..." << endl;
        for (auto &queue : S::command_queue)
            queue.clear();
        for (int i = 0; i < S::gs.nanogrid_pos.size(); i++)
        {
            pos_to_nanogrid[S::gs.nanogrid_pos.at(i)] = i;
        }
        int place_num = min(S::EV.N_EV, S::gs.nanogrid_pos.size());
        vector<size_t> EV;
        vector<size_t> nanogrid;
        map<size_t, size_t> pos_to_id;
        map<size_t, size_t> EV_to_nanogrid;
        auto loss = calc_loss(S::grid);
        auto matching = mincost_matching(S::EV.pos, S::gs.nanogrid_pos, S::gs, loss);
        for (size_t i = 0; i < matching.size(); i++)
        {
            size_t idx_EV = matching[i].first;
            size_t idx_nanogrid = matching[i].second;
            size_t from = S::EV.pos.at(idx_EV);
            size_t to = S::gs.nanogrid_pos.at(idx_nanogrid);
            auto move = move_EV(from, to, S::gs);
            S::enqueue(idx_EV, move_EV(from, to, S::gs));
            assert(move_EV(from, to, S::gs).size() == 0 || S::is_free(idx_EV) == false);
        }
    }
    void command(const grid_info &g_i, const EV_info &ev_i, const order_info &)
    {
        map<size_t, size_t> offset_EV;
        map<size_t, size_t> offset_grid;
        for (int i = 0; i < g_i.y.size(); i++)
        {
            total_loss += g_i.pw_excess[i];
            total_buy -= g_i.pw_buy[i];
        }
        cerr << "@ " << 0 << " " << total_loss << endl;
        cerr << "@ " << 1 << " " << total_buy << endl;
        for (size_t n = 0; n < ev_i.N_EV; ++n)
        {
            ...
            assert(pos == current);
            size_t nanogrid_idx = pos_to_nanogrid[pos];
            size_t nanogrid_charge = g_i.y[nanogrid_idx] + offset_grid[nanogrid_idx];
            size_t EV_charge = ev_i.c[n].charge + offset_EV[n];
            if (nanogrid_charge < 0.5 * (double)S::grid.C_grid_max)
            // if (g_i.pw_actual[nanogrid_idx] < 0)
            {
                size_t charge_V = min({S::EV.V_EV_max, EV_charge, S::grid.C_grid_max - nanogrid_charge});
                if (charge_V > 0)
                {
                    DUMP(charge_V);
                    DUMP(S::EV.C_EV_max - EV_charge);
                    DUMP(S::EV.C_EV_max);
                    DUMP(EV_charge);
                    DUMP(nanogrid_charge);
                    S::enqueue(n, strprintf("charge_to_grid %zu", charge_V), 1);
                    offset_EV[n] -= charge_V;
                    offset_grid[nanogrid_idx] += charge_V;
                }
                else
                {
                    S::enqueue(n, "stay", 1);
            ...
            {
                size_t charge_V = min({S::grid.V_grid_max, S::EV.V_EV_max, nanogrid_charge, S::EV.C_EV_max - EV_charge});
                if (charge_V > 0)
                {
                    DUMP(charge_V);
                    DUMP(S::EV.C_EV_max - EV_charge);
                    ...
                    offset_EV[n] += charge_V;
                    offset_grid[nanogrid_idx] -= charge_V;
                    S::enqueue(n, strprintf("charge_from_grid %zu", charge_V), 1);
                }
                else
                {
                    S::enqueue(n, "stay", 1);
...
int main()
{
    ...
    {
        str.reset(new EV_is_battery<A>(prob, gs));
