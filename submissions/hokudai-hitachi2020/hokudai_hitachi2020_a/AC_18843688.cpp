...
auto mincost_matching(const vector<size_t> &EV, const vector<size_t> &nanogrid, const graph_summary &gs)
{
    ...
    for (int i = 0; i < EV.size(); i++)
    {
        ...
        {
            g.add_edge(i, offset + j, 1, gs.len[EV[i]][nanogrid[j]]);
...
struct EV_is_battery : strategy<P>
{
    ...
    void initialize()
    {
        ...
        auto matching = mincost_matching(S::EV.pos, S::gs.nanogrid_pos, S::gs);
