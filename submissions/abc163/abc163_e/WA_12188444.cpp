...
int main()
{
    ...
    rep(i, n)
    {
        ...
        dp[i + 1][0] = dp[i][0] + abs(p[i].second - i) * p[i].first;
        dp[0][i + 1] = dp[0][i] + abs(n - 1 - i - p[i].second) * p[i].first;
    }
    cerr << dp[n][0] << endl;
    //    cerr << dp[0][n] << endl;
    rep(i, 1, n)
    {
        rep(j, 1, n)
        {
            ...
            dp[i][j] = max(dp[i - 1][j] + abs(p[i + j - 1].second - (i - 1)) * p[i + j - 1].first, dp[i][j - 1] + abs(n - 1 - (j - 1) - p[i + j - 1].second) * p[i + j - 1].first);
        }
    }
    ll ans = 0;
    rep(i, n + 1)
