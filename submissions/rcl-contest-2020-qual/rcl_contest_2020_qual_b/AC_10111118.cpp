...
vector<int> pick(int turn, int prevy, int prevx, vector<vector<int>> &g, vector<vector<int>> paintable, vector<vector<int>> &pict)
{
    int n = 50;
    ...
    rep(i, n) rep(j, n)
    {
        if ((pict[i][j] == turn % 4) && g[i][j] != turn % 4)
        {
            newy = i;
            newx = j;
            break;
    ...
    //cerr << newy << " " << newx << endl;
    int rstart = mt() % paintable.size();

    rep(ii, paintable.size())
    {
        // cerr << ii << endl;
        r = (rstart + ii) % paintable.size();
        //mt() % paintable.size();
        py = paintable[r][0];
        px = paintable[r][1];
        col = g[py][px];
        ...
        if (col != turn % 4)
            continue;
        assert(newy == -1 || col == pict[newy][newx]);
        int bestdir;
        int bestnum = -(1 << 30);
        rep(dir, 4)
        {
            int curnum = 0;
            int dist = abs(newy - py) + abs(newx - px);
            rep(j, 1, 6)
            {
                int ny = py + j * dy[dir];
                int nx = px + j * dx[dir];
                if (is_valid(ny, nx))
                {
                    ...
                    if (g[ny][nx] == -1)
                        curnum += 10;
                    if (g[ny][nx] == pict[ny][nx])
                        curnum -= 10;
                    //dist = min(dist, abs(newy - ny) + abs(newx - nx));
                    dist = abs(newy - ny) + abs(newx - nx);
                }
            }
            curnum += mt() % 20;
            if (newy != -1)
...
void genTactics(Tactics &tact, vector<vector<int>> &pict)
{
    ...
    rep(i, m)
    {
        ...
        int col = i % 4;
        auto bestmove = pick(i, prevy, prevx, g, paintable, pict);
        int py = bestmove[0];
        int px = bestmove[1];
        int dir = bestmove[2];
        rep(j, 1, 6)
